...
### 6.6 결제 트랙 분리 (2026-05-21 도입)
하루루트 결제는 두 트랙으로 분리한다.

1. **월 구독 (subscription)** — ₩9,900/월, 무제한 열람. 결제일 + 30일 만료, 자동 갱신.
2. **일회성 구매 (one-time pass)** — 1회 ₩990 / 3회 ₩2,500 / 5회 ₩3,600. 발급 후 **90일** 만료.

규칙:
- 일회성 패스는 `one_time_passes` 테이블에서 잔여 횟수(`remaining_uses`)와 만료(`expires_at`) 관리. 데이터 모델 상세는 `DATA_MODEL_API.md` §2.10b 참고.
- 잠금해제 시 entitlement 우선순위: **활성 구독 > 잔여 패스(만료 임박 순) > 결제 시트 열기**.
- 같은 사용자가 동일 루트를 재열람할 때는 횟수 차감 없음 (`route_unlocks` 별도 테이블로 1:1 영속화).
- 환불: 디지털 콘텐츠 특성상 미사용 패스만 7일 이내 환불 가능. 1회라도 사용된 패스는 환불 불가.
- 가격·할인은 DB에 시드(`product_prices` 또는 코드 상수)로 관리하여 A/B 테스트 여지 확보.
