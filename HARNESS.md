...
- **AI 협업·UX·보고:** [AI_DEVELOPMENT_RULES.md](./AI_DEVELOPMENT_RULES.md) — Plan/Check, UX·개발 금지, 검증·완료 보고 형식. **작업 종료 시 [docs/dev-log.md](./docs/dev-log.md) 기록은 필수.** **이 문서(HARNESS)·AGENTS·FOUNDATION·DATA_MODEL_API 등을 개정할 때는 본 규칙과의 정합성을 반드시 확인한다.**
...
에이전트·인간 모두 PR·채팅 마무리 시 아래를 맞춘다. **저장소에는 [docs/dev-log.md](./docs/dev-log.md)에 동일 작업을 날짜·형식대로 남긴다(필수).** 상세 10항목 체크리스트는 [AI_DEVELOPMENT_RULES.md](./AI_DEVELOPMENT_RULES.md) **보고 형식**을 사용한다(아래는 최소 공통).
...
- [docs/dev-log.md](./docs/dev-log.md) — 작업 완료 후 필수 기록
