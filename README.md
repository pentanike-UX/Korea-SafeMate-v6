...
Supabase·지도·가디언 포스트 API·**네이버 검색(하루웨이 스팟 보강)**·**Google Places(스팟 place_id 갤러리)** 등은 해당 기능을 켤 때만 필요합니다. 이름·용도는 루트 **`env.example`** 을 본다(시크릿은 커밋하지 않음). 네이버 검색은 서버 전용(`NAVER_SEARCH_*`). Google은 키 2종 — 서버 `GOOGLE_MAPS_API_KEY`(Places Text/Details/Photos) + 브라우저 `NEXT_PUBLIC_GOOGLE_MAPS_BROWSER_KEY`(지도 JS + 클라이언트 Places Autocomplete; HTTP referrer 제한 권장). [DATA_MODEL_API.md](./DATA_MODEL_API.md) §5.3·§10, [HARNESS.md](./HARNESS.md) §4.
