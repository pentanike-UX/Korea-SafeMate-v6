...
/** 대표 스팟 1~2개 한 줄 — `venueSafe`면 분류형 명칭만(실명·주소 없음) */
export function routeCardSpotPreviewLine(
  post: ContentPost,
  max = 2,
  opts?: { venueSafe?: boolean },
): string | null {
  ...
  return sorted
    .slice(0, max)
    .map((s) => (opts?.venueSafe ? freeClassificationTitle(s) : s.real_place_name || s.display_name || s.place_name))
    .join(" · ");
}
